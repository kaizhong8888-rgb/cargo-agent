...
impl Tool for TemplateTool {
    ...
    fn parameters(&self) -> Vec<ToolParameter> {
        vec![
            tp("action", "Action: render, render_file, render_with_data", true),
            tp("template", "Template string content (for render action)", false),
            tp("template_file", "Path to template file (for render_file)", false),
            tp("data", "JSON string with template variables", false),
            tp("data_file", "Path to JSON data file", false),
            tp("output", "Output file path to write result (optional)", false),
        ]
    }
...
fn render(params: &HashMap<String, Value>) -> Result<Value, String> {
    let template = params.get("template").and_then(|v| v.as_str()).ok_or("template is required")?;
...
fn render_file(params: &HashMap<String, Value>) -> Result<Value, String> {
    let template_file = params.get("template_file").and_then(|v| v.as_str())
...
fn render_with_data(params: &HashMap<String, Value>) -> Result<Value, String> {
    let template = params.get("template").and_then(|v| v.as_str()).ok_or("template is required")?;
    let data_file = params.get("data_file").and_then(|v| v.as_str())
    ...
    let data: Value = serde_json::from_str(&data_raw)
        .map_err(|e| format!("Invalid JSON in data file: {e}"))?;
