...
impl Tool for CronTool {
    ...
    fn parameters(&self) -> Vec<ToolParameter> {
        vec![
            ...
            ToolParameter {
                ...
                description: "Number of next run times to compute (default: 5, max: 20)"
                    .to_string(),
                required: false,
            },
        ]
    ...
    async fn execute(&self, params: &HashMap<String, Value>) -> Result<Value, String> {
        ...
        match action {
            ...
            _ => Err(format!(
                "Unknown action: {action}. Valid: parse, next, validate, describe"
            )),
        }
    }
}
...
fn next_run_times(params: &HashMap<String, Value>) -> Result<Value, String> {
    ...
    current = current
        .with_second(0)
        .expect("second=0 should always be valid");
...
fn describe_cron(params: &HashMap<String, Value>) -> Result<Value, String> {
    ...
    {
        format!(
            "At minute 0 past hour {}, {}",
            &fields[1].original, description
        )
    } else {
        description
    };
...
fn parse_cron_expression(expr: &str) -> Result<Vec<CronField>, String> {
    ...
    if parts.len() != 5 {
        return Err(format!(
            "Expected 5 fields, got {}. Format: 'minute hour day_of_month month day_of_week'",
            parts.len()
        ));
    }
...
fn parse_cron_field(field: &str, min: u32, max: u32) -> Result<Vec<u32>, String> {
    ...
    for part in field.split(',') {
        ...
        if part == "*" {
            ...
            if step == 0 || step > max {
                return Err(format!("Step {step} out of range [{min}-{max}]"));
            }
            let mut v = min;
            while v <= max {
                values.push(v);
            ...
            if range_part.contains('-') {
                ...
                let start: u32 = range_parts[0]
                    .parse()
                    .map_err(|_| format!("Invalid start in '{part}'"))?;
                let end: u32 = range_parts[1]
                    .parse()
                    .map_err(|_| format!("Invalid end in '{part}'"))?;
                if start > end {
...
mod tests {
    ...
    fn test_describe_weekdays() {
        ...
        p.insert(
            "expression".to_string(),
            Value::String("0 0 * * 1-5".to_string()),
        );
        let r = describe_cron(&p).unwrap();
