...
use config::{parse_checks, Thresholds};
...
impl Tool for CodeReviewTool {
    ...
    async fn execute(&self, params: &HashMap<String, Value>) -> Result<Value, String> {
        ...
        let show_progress = merged_params
            ...
            .and_then(|v| v.as_bool())
            .unwrap_or(true);
        let git_diff_flag = merged_params
            .get("git_diff")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);
        ...
        if files.is_empty() {
            return Ok(build_empty_result(git_diff_flag));
        }

        let (all_issues, file_summaries, total_errors, total_warnings, total_info) = analyze_batch(
            &files,
            &active_checks,
            &thresholds,
            min_severity,
        ...
        let mut result = generate_report(
            &all_issues,
            &file_summaries,
            &files,
            total_errors,
