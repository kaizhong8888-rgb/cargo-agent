...
impl Tool for ProcessTool {
    ...
    fn parameters(&self) -> Vec<ToolParameter> {
        vec![
            ...
            ToolParameter {
                ...
                parameter_type: "string".to_string(),
                description: "Signal to send: term (default), kill, int (for kill action)"
                    .to_string(),
                required: false,
            },
        ]
    ...
    async fn execute(&self, params: &HashMap<String, Value>) -> Result<Value, String> {
        ...
        match action {
            ...
            _ => Err(format!(
                "Unknown action: {action}. Valid: list, info, kill, tree, search"
            )),
        }
    }
}
...
fn kill_process(params: &HashMap<String, Value>) -> Result<Value, String> {
    ...
    let killed = match signal {
        ...
        "int" | "2" => {
            #[cfg(unix)]
            {
                unsafe { libc::kill(pid_val as i32, libc::SIGINT) == 0 } // SAFETY: `libc::kill` is a standard POSIX syscall. `pid_val` was validated via `sys.process(pid)`.
            }
            #[cfg(not(unix))]
            {
                process.kill()
...
fn build_tree_node(
    ...
    *count += 1;
    ...
    node.insert(
        "cpu_usage".to_string(),
        Value::Number(serde_json::Number::from_f64(process.cpu_usage() as f64).unwrap_or(0.into())),
    );
    node.insert(
        "memory_display".to_string(),
        Value::String(format_memory(process.memory())),
    );
    if let Some(child_pids) = children.get(&pid) {
        ...
        for child_pid in child_pids {
            ...
            if let Some(child_node) =
                build_tree_node(*child_pid, sys, children, count, limit, depth + 1)
            {
