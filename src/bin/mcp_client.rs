...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 你也可以将 npx 替换为其他 MCP Server 的可执行文件路径
    ...
    if let Some(tools) = list_resp.get("result").and_then(|v| v.get("tools")) {
        println!("✅ 发现以下工具:");
    ...
    // 解析返回内容
    if let Some(content) = call_resp.get("result").and_then(|v| v.get("content")) {
